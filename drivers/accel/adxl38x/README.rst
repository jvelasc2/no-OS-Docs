...
|                     |                     |                     | much functionality. |
+---------------------+---------------------+---------------------+---------------------+
| ADXL38X_MODE_LP_SER | Low power serial    | 6                   | Use in scenarios    |
| _ULP_OP             | ultra-low power     |                     | where serial        |
|                     | operation mode.     |                     | communication is    |
|                     |                     |                     | key while           |
|                     |                     |                     | maintaining ultra-  |
|                     |                     |                     | low power.          |
+---------------------+---------------------+---------------------+---------------------+
| ADXL38X_MODE_LP_SER | Low power serial    | 7                   | For applications    |
| _VLP_OP             | very low power      |                     | requiring serial    |
...
|                     |                     |                     | capabilities.       |
+---------------------+---------------------+---------------------+---------------------+
| ADXL38X_MODE_RBW_SER| Reduced bandwidth   | 10                  | Balancing minimal   |
| _ULP_OP             | serial ultra-low    |                     | bandwidth when      |
|                     | power operation.    |                     | serial communication|
|                     |                     |                     | and ultra-low power |
|                     |                     |                     | modes are required. |
+---------------------+---------------------+---------------------+---------------------+
| ADXL38X_MODE_RBW_SER| Reduced bandwidth   | 11                  | Use in high-        |
| _VLP_OP             | serial very low     |                     | efficiency scenarios|
...
|                     |                     |                     | constraint.         |
+---------------------+---------------------+---------------------+---------------------+
| ADXL38X_MODE_HP_SER | High power serial   | 14                  | Combines high       |
| _ULP_OP             | ultra-low power     |                     | performance with    |
|                     | operation mode.     |                     | ultra-low power in  |
|                     |                     |                     | serial              |
|                     |                     |                     | communications.     |
+---------------------+---------------------+---------------------+---------------------+
| ADXL38X_MODE_HP_SER | High power serial   | 15                  | Maximizing          |
| _VLP_OP             | very low power      |                     | performance with    |
...
.. code-block:: C
   ...
   struct no_os_spi_init_param adxl38x_spi_ip = {
       ...
       init_param.comm_type = ADXL38X_SPI_COMM;  // Use SPI in this example
       init_param.comm_init.spi_init = (struct no_os_spi_init_param){
           // SPI initialization parameters such as chip select, mode, etc.
       };

       // Device is now initialized, proceed with operations
